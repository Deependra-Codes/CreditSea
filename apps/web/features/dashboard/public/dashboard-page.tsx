...
import type { LeadResponse, LoanResponse, OverviewResponse, PublicUser } from "@lms/contracts";
...
export async function DashboardPage({ module }: { module: string }) {
  ...
  return (
    <DashboardShell user={session.user} current={module}>
      {await sectionContent(module)}
    </DashboardShell>
  );
}
